...
{
    public partial class FindCars : Form
    {
        public FindCars()
        {
            InitializeComponent();
        }
        // Veri tabanı işlemleri için bağlantı nesnesi
        ...
        {
            // Tüm otomobil markalarını listelemek için sorgu
            connect.Open();
            FrmListCars newform = new FrmListCars();
            SqlDataAdapter da = new SqlDataAdapter("Select ilan_no AS 'İlan Numarası',ilan_tarihi AS 'İlan Tarihi',fiyat AS 'Fiyat',yil AS 'Yıl',yakit AS 'Yakıt',arac_durumu AS 'Araç Durumu',km AS 'KM',kasa_tipi AS 'Kasa Tipi',motor_gucu AS 'Motor Gücü',motor_hacmi AS 'Motor Hacmi',renk AS 'Renk',aciklama AS 'Açıklama',marka AS 'Marka',model AS 'Model',seri AS 'Seri',vites AS 'Vites',gallery_name AS 'Galeri İsmi' FROM ilan INNER JOIN gallery_Register on ilan.gallery_user_name=gallery_Register.gallery_user_name", connect);
            da.Fill(newform.dt);
            SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM ilan",connect);
            SqlDataReader read = cmd.ExecuteReader();
            while (read.Read())
            {
                newform.total_record = Convert.ToInt32(read[0]);
            }
            newform.brand = "";
            newform.model = "";
            newform.serie = "";
        ...
        //Markaya göre serileri getirme
        private void comboBox1_TextChanged(object sender, EventArgs e)
        {
            linkLabel1.Text = "Tüm" + " " + comboBox1.Text + " " + "İlanları";
            string[] Acura = { "RSX" };
            string[] Aion = { "S" };
        ...
        {
            // Sadece markaya göre ilan getirme
            connect.Open();
            FrmListCars newform = new FrmListCars();
            SqlCommand cmd = new SqlCommand("Select *From ilan where marka=@p1", connect);
            cmd.Parameters.AddWithValue("@p1", comboBox1.Text);
            SqlDataAdapter da = new SqlDataAdapter();
            da.SelectCommand = cmd;
            da.Fill(newform.dt);
            SqlCommand cmd2 = new SqlCommand("SELECT COUNT(*) FROM ilan where marka=@p1", connect);
        ...
        {
            linkLabel2.Text = "Tüm" + " " + comboBox2.Text + " " + "İlanları";
        ...
        {
            linkLabel3.Text = "Tüm" + " " + comboBox3.Text + " " + "İlanları";
        }

        private void linkLabel2_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            // istenilen marka ve seriye göre listeleme
            connect.Open();
            FrmListCars newform = new FrmListCars();
            SqlCommand cmd = new SqlCommand("Select *From ilan where marka=@p1 and seri=@p2", connect);
            cmd.Parameters.AddWithValue("@p1", comboBox1.Text);
            cmd.Parameters.AddWithValue("@p2", comboBox2.Text);
            SqlDataAdapter da = new SqlDataAdapter();
            da.SelectCommand = cmd;
            da.Fill(newform.dt);
            SqlCommand cmd2 = new SqlCommand("SELECT COUNT(*) FROM ilan where marka=@p1 and seri=@p2", connect);
            cmd2.Parameters.AddWithValue("@p1", comboBox1.Text);
            cmd2.Parameters.AddWithValue("@p2", comboBox2.Text);
            SqlDataReader read = cmd2.ExecuteReader();
            while (read.Read())
            {
                newform.total_record = Convert.ToInt32(read[0]);
        ...
        {
            // istenilen marka model ve seriye göre listeleme
            connect.Open();
            FrmListCars newform = new FrmListCars();
            SqlCommand cmd = new SqlCommand("Select *From ilan where marka=@p1 and seri=@p2 and model=@p3", connect);
            cmd.Parameters.AddWithValue("@p1", comboBox1.Text);
            cmd.Parameters.AddWithValue("@p2", comboBox2.Text);
            ...
            da.SelectCommand = cmd;
            da.Fill(newform.dt);
            SqlCommand cmd2 = new SqlCommand("SELECT COUNT(*) FROM ilan where marka=@p1 and seri=@p2 and model=@p3", connect);
            cmd2.Parameters.AddWithValue("@p1", comboBox1.Text);
            cmd2.Parameters.AddWithValue("@p2", comboBox2.Text);
            cmd2.Parameters.AddWithValue("@p3", comboBox3.Text);
            SqlDataReader read = cmd2.ExecuteReader();
            while (read.Read())
            {
                newform.total_record = Convert.ToInt32(read[0]);
            ...
            newform.brand = comboBox1.Text;
            newform.model = comboBox2.Text;
            newform.serie = comboBox3.Text;
            newform.Show();
            connect.Close();
        }

        // Form yüklendiğinde linklabeller boş gelir

        private void FindCars_Load(object sender, EventArgs e)
        {
            linkLabel1.Text = "";
            linkLabel2.Text = "";
            linkLabel3.Text = "";
        ...
        {
            // İstenilen filtrelemeye göre listeleme yapma
            connect.Open();
            FrmListCars newform = new FrmListCars();
            SqlCommand cmd = new SqlCommand("Select *From ilan where marka=@p1 and seri=@p2 and model=@p3", connect);
            cmd.Parameters.AddWithValue("@p1", comboBox1.Text);
